...
/*! \brief Free an Amoeba record.
 *
 */
void AL1_FreeAmoeba(struct Cytoskelenton *C){
  unsigned long int A=0,B=0;
  struct Pseudopod     *FP=NULL;
  struct Mitochondrion *FM=NULL;
  if(C){
...
/*! \brief Update a record in the Amoeba database.
 *
 */
int AL1_RepDBEntry(struct Colony *City,struct Cytoskelenton *Data,unsigned long int RecordID){  
  unsigned long int A=0,B=0,S=0,Pos=0,SizeWrite=0,WriteTo=0,StartPos=0,FinishPos=0;
...
/*! \brief Add a new record in the Amoeba database.
 *
 */
int AL1_AddDBEntry(struct Colony *City,struct Cytoskelenton *Data){
  unsigned long int A=0,B=0,S=0,Pos=0,SizeWrite=0,WriteTo=0,Location=0;
  SizeWrite=(sizeof(struct Cytoskelenton)+SHMSPACING)+Data->Size+SHMSPACING+Data->Dimension*(sizeof(struct Pseudopod)+SHMSPACING);
  for(A=0;A<Data->Dimension;A++)
    S+=Data->Pseudopods[A]->Dimension;
  SizeWrite+=S*(sizeof(struct Mitochondrion)+SHMSPACING);
...
/*! \brief Defragment the allocation table.
 *
 */
void AL1_DefragAllocTable(struct Colony *City){
  unsigned long int _=0,Position=0,Gaps=0,*Fragments=NULL;
...
/*! \brief Relocate the position of a record within the Amoeba database.
 *
 */
void AL1_Relocate(struct Colony *City,unsigned long int Gap,unsigned long int From){
  unsigned long int _=0,A=0,B=0,C=0,Start=0,End=0;
  struct Blob *Name=NULL;
...
/*! \brief Rename an Amoeba record.
 * - Note that anything connected to this record will see the change instantly.
 */
...
/*! \brief Retrieve the record name which is also the primary key.
 * 
 */
struct Blob *AL1_GetRecName(struct Colony *City,unsigned long int RecordID){
  ...
  if(RecordID<City->Population){
    ...
    printf("CRITICAL ERROR: Unable to find record: [%ld] of [%ld]\n",RecordID,City->Population);
    /* we need a better solution than this */
    R->Size=0;
    R->Nucleus=StrCat("","-> FAULT <-");
  }
  return R;
}

/*! \brief Reread the header data for Amoeba so that SHM information can be read.
 * 
 */
void AL1_ReadHeader(struct Colony *City){
  unsigned long int Gamma=0,A=0;
...
/*! \brief Write the header data for Amoeba so that SHM information can be read.
 * 
 */
...
/*! \brief Initialise the header data to sensible defaults so that Amoeba can be read.
 * 
 */
...
/*! \brief Remove an Amoeba record from the database.
 * 
 */
int AL1_DelDBEntry(struct Colony *City,unsigned long int RecordID){
  ...
  if((RecordID<City->Population)&&(RecordID)){
    ...
    /* should set the record to link back to the first blank record */
...
/*! \brief This is one of the search threads.
 * 
 */
void *AL1_SearchThread(void *Data){
  unsigned long int _=0,Pos=0;
  struct Blob *Name=NULL;
  struct SearchThreadData *D=(struct SearchThreadData *)Data;
...
/*! \brief Forks off multiple search threads and looks for data within the database.
 *  - Threads AL1_SearchThread().
 */
...
/*! \brief Populates the SHM with initial information.
 *  
 */
void AL1_CreateColony(struct Colony *City){
  int *SHMId=NULL;
  unsigned long int *Size=NULL;
  key_t *Key=NULL;
  Size=&City->Size;
  SHMId=&City->SHMId;
  Key=&City->Key;
...
/*! \brief Nukes the Amoeba database.
 *  
 */
...
/*! \brief Reads the data from disk and loads it into Amoeba.
 *  
 */
...
/*! \brief Writes the Amoeba database to disk.
 *  
 */
...
/*! \brief Obtain an Amoeba record from its position in SHM.
 *  
 */
struct Cytoskelenton *AL1_GetDBEntry(struct Colony *City,unsigned long int RecordID){
  unsigned long int A=0,B=0,Pos=0;
  char *Nucleus=NULL;
  struct Cytoskelenton *C =(struct Cytoskelenton *)malloc(sizeof(struct Cytoskelenton));
  struct Pseudopod     *P=NULL;
  struct Mitochondrion *M=NULL;
...
/*! \brief Convert pointer addresses from external sources so they can be used internally.
 *  
 */
...
/*! \brief Convert pointer addresses from internal sources so they can be saved externally.
 *  
 */
...
/*! \brief Defragment the data area of Amoeba.
 *  
 */
void AL1_Defrag(struct Colony *City){ /* Bug with deleted records */
  unsigned long int _=0,Fragmented=0,C=0,A=0,B=0,OldB=0,LastPos=0,F=0,G=0;
...
/*! \brief Update the Amoeba header information that controls the DB parameters.
 *  
 */
...
/*! \brief Retrieve the header start position.
 *  
 */
...
/*! \brief Retrieve the header finish position.
 *  
 */
...
/*! \brief Retrieve the cache start position.
 *  
 */
...
/*! \brief Apply recaching record "ID" with geometric ratio 0.5.
 *  
 */
...
/*! \brief Remove header entry for an Amoeba record.
 *  
 */
void AL1_DelIDHeader(struct Colony *City,unsigned long int ID,unsigned long int Cached){
  unsigned long int MaxPos=((FATSIZE-FATSTART)/3),A=0,B=0,C=0;
